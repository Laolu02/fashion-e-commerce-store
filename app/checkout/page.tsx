...
async function handlePayment(formData: FormData) {
  ...
  const authUrl = await initializePaystackTransaction(
    session.user.email!, 
    totalAmount, 
    reference,
    callbackUrl
...
export default async function Checkout() {
  ...
  // Extract only serializable data for client-side rendering
      ...
      <div className="w-full max-w-md bg-white p-8 lg:p-12 shadow-[0_20px_50px_rgba(0,0,0,0.1)] relative border-t-8 border-primary">
        ...
        <div className="space-y-2 mb-12">
          {cartItems.map((item, i) => (
            <div key={i} className="flex justify-between text-xs font-bold text-primary/60 uppercase">
              <span>{item.name} x {item.quantity}</span>
              <span>₦{(item.price * item.quantity / 100).toLocaleString('en-NG')}</span>
            </div>
          ))}
