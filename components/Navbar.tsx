...
import { Search, ShoppingBag, LogOut, Settings, Package, Menu, X, ArrowUpRight, ShieldCheck } from 'lucide-react';
        ...
        <div className="flex justify-between items-center h-16">
          <Link href="/" className="shrink-0 group">
            <div className="text-lg font-black text-primary uppercase tracking-tighter leading-tight">
              ATELIER
              <span className="text-primary/20 group-hover:text-primary transition-colors duration-500">
                _ARCHIVE
              </span>
            </div>
          </Link>
          <div className="hidden md:flex flex-1 max-w-37.5 lg:max-w-xs xl:max-w-md mx-6 lg:mx-12">
            <form onSubmit={handleSearch} className="relative w-full group">
              <input
                ...
                placeholder="SEARCH"
                className="w-full bg-transparent border-b border-primary/500 py-1 text-[8px] font-black uppercase tracking-[0.3em] focus:border-primary focus:outline-none transition-all placeholder:text-primary/30"
              />
              <button
                type="submit"
                className="absolute right-0 top-1/2 -translate-y-1/2"
              >
                <Search className="w-3 h-3 text-primary/60 group-focus-within:text-primary transition-colors" />
              </button>
            </form>
          </div>
          <div className="flex items-center gap-4 lg:gap-10">
            ...
            <div className="flex items-center gap-2 lg:gap-6">
              <Link
                href="/cart"
                className="relative group flex items-center gap-2 py-2 px-1 lg:px-2"
              >
              ...
              {session ? (
                <div className="relative">
                  ...
                  {isUserMenuOpen && (
                    <div
                      className="absolute right-0 mt-4 w-56 bg-white border-2 border-primary z-50 animate-in fade-in slide-in-from-top-2"
                    ...
                    >
                      <div className="p-4 border-b border-primary/10 bg-neutral-50">
                        ...
                        {session.user?.role === "ADMIN" && (
                          <p className="text-[7px] font-mono font-bold text-primary/40 mt-1 uppercase tracking-tighter">
                            Auth_Level: 01_ADMIN
                          </p>
                        )}
                      </div>
      ...
      {isMobileMenuOpen && (
        <div className="fixed inset-0 bg-white z-150 flex flex-col p-10 pt-24 animate-in slide-in-from-right duration-500">
          <button
            ...
            className="absolute top-6 right-6 p-3 border border-primary"
          >
          ...
          <div className="flex flex-col space-y-8">
            <form
              ...
              className="mb-8 relative border-b-2 border-primary"
            >
              <input
                type="text"
                value={searchQuery}
            ...
            <Link
              href="/products"
              className="text-5xl font-bold uppercase tracking-tighter text-primary"
              onClick={() => setIsMobileMenuOpen(false)}
            >
              Shop_
            </Link>
